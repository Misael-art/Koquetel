...
| G-03 | high | Local structural audit is complete for the selected core files; external specialized implementations remain documentation-level | equivalent pinned structural audits for any implementation selected as base | final dependency selection |
| G-04 | high | ai-memory durability, concurrency, export and corruption recovery are unproven | fault-injection prototype and recovery report | memory backend acceptance |
