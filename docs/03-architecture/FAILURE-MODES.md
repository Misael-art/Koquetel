...
| FM-23 | v1 local lease holder stuck/paused (SIGSTOP, D-state, deadlock) keeps the OFD lock | holder liveness/heartbeat staleness surfaced by `status`/`doctor`; contenders block on `F_OFD_SETLKW` | surface the stall; the operator terminates the stuck holder (SIGKILL releases the OFD, enabling takeover). No automatic force-takeover — that would risk two writers. This is the residual of ADR-0011 Decision A (local model) | **liveness stall only** — never two concurrent writers, no state corruption; converges to a usable state once the stuck holder is terminated |
